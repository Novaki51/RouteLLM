...
class TextRoutePlanner:
    def __init__(self, master):
        self.master = master
        self.master.title("OSMnx Route Planner - Export XY CSV and Length")
        ...
        # Output area for X-Y and length
        ttk.Label(self.right, text="Route X-Y coordinates and length:").pack(pady=(15, 2))
        self.xy_text = tk.Text(self.right, height=12, width=36, state=tk.DISABLED)
        self.xy_text.pack(pady=(0, 8))

        # State variables
        self.xys = None          # Holds last planned route coordinates
        self.length_m = None     # Holds last planned route length in meters
        self.base_graph = None   # The unprojected graph
    ...
    def show_xy_coords(self, xys):
        ...
        if not xys:
            ...
            # Header with units
            self.xy_text.insert(tk.END, "     X (m)\t     Y (m)\n")
            for x, y in xys:
                self.xy_text.insert(tk.END, f"{x:.2f} m\t{y:.2f} m\n")
            # Total length if available
            if self.length_m is not None:
                self.xy_text.insert(tk.END, f"\nTotal length: {self.length_m:.2f} m\n")
    ...
    def plan_route(self):
        ...
        try:
            ...
            # Project to Budapest UTM zone (EPSG:32634) for meter accuracy
            G_proj = ox.project_graph(G, to_crs="EPSG:32634")
            ...
            lengths = []
            for u, v in zip(route_proj[:-1], route_proj[1:]):
                # get edge data for each pair (could be multi-edge)
                edge_data = G_proj.get_edge_data(u, v)
                # choose the minimal length among parallel edges
                seg_length = min(attr.get('length', 0) for attr in edge_data.values())
                lengths.append(seg_length)
            self.length_m = sum(lengths)
            ...
            # Enable save and display coords+length
